...
import { clearUser, selectUserState } from "app/UserReducer";
import defaultImg from 'images/default_user_img.png'
...
export default function AvatarDropdown() {
  ...
  const { name } = useAppSelector(selectUserState)
  ...
  useEffect(() => {
    ...
    axios.get(url)
      .then(res => setGravatar(url))
      .catch(err => setGravatar(defaultImg))
  }, [name])

  const clickDisconnectUser = () => {
    disconnectUser(dispatch, disconnect)
                ...
                <div className="overflow-hidden rounded-3xl shadow-lg ring-1 ring-black ring-opacity-5">
                  <div className="relative grid grid-cols-1 gap-6 bg-white dark:bg-neutral-800 py-7 px-6">
                    <div className="flex items-center space-x-3">
                      <Avatar imgUrl={gravatar} sizeClass="w-12 h-12" />

                      <div className="flex-grow overflow-hidden">
                        <h6 className="font-semibold text-sm overflow-hidden truncate">{name}</h6>
