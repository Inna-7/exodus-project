...
import pics71 from "images/exo_pics/Pics_71.jpg"
...
import { getNfts, getCurrencySet, selectNftState } from "app/nftsState/nftsReducer";
...
const PageCollection: FC<PageCollectionProps> = ({ className = "" }) => {
  ...
  return (
    ...
    >
      <Helmet>
        <title>Collection || Exodus Metaverse NFT Template</title>
      </Helmet>
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-x-8 gap-y-10  mt-8 lg:mt-10">
            {nfts?.map((nft: any) =>
              <CardNFT_V2
                key={nft.id}
                name={nft.name}
                type={!nft.image ? "video" : "image"}
                src={`https://ipfs.io/ipfs/${nft.image || nft.video}`}
                price={nft.price}
                id={nft.id}
              />
            )}
